...
async function indexCoinbaseBlock() {
  const coinbaseTx = makeCoinbaseTx(0, 50 * SATOSHI);
  block0 = makeBlock(0, '00'.repeat(32), [coinbaseTx]);
  await processAndCommit(tracker, block0);
}

function registerIndexedReadTests() {
...
function registerIndexedSpendTests() {
...
function indexedBlockTests() {
  before(indexCoinbaseBlock);
  registerIndexedReadTests();
  registerIndexedSpendTests();
}

function registerIndexedBlockTests() {
  describe('after indexing one coinbase block', indexedBlockTests);
}
...
function expressApiTests() {
  before(buildExpressApp);
...
function registerExpressApiTests() {
  describe('Express API layer', expressApiTests);
}
...
function smokeTests() {
  ...
  registerIndexedBlockTests();
  registerExpressApiTests();
}

describe('UTXO Tracker Smoke Tests', smokeTests);
