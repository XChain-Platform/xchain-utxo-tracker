...
// inputs) need a freshness signal on the response itself: the API exposes
// committed height / node tip / lag / synced, computed by the shared
...
describe('computeFreshness', function () {
  ...
  it('lets no route harness rebuild the header from the raw sync flag', function () {
    const harnesses = ['../../security/rest_route_surface.test.js', '../../unit/api.test/support/test_app.js'];
...
//
// A failed bootstrap compression must not freeze the tracker (parsing stopped,
// never relaunched), and a failed restore must not strand a wiped /data,
// silently. Bootstrap failure resumes indexing and keeps the task
