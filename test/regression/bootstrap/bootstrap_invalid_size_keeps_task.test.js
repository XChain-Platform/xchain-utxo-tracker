...
// compressDirPigz's invalid-du-size branch must not `delete tasks[taskId]`
// before throwing: that drops progress -1 and the error, leaving
// getbootstrapstatus answering "taskid doesn't exist" rather than the real
// failure. Other regression tests for this task-record class all
// call handleBootstrapFailure with the record already present, which is
// exactly the precondition a premature delete violates, so none of them catch it.
//
