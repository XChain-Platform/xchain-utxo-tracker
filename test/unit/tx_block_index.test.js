...
describe('LevelUpDb tx block index (T prefix)', function () {
  ...
  it('returns block_hash, block_height and sync on an exact hit', async function () {
  ...
  it('returns null for a different txid sharing the stored 8-byte prefix', async function () {
  ...
  it('deletes the T record for every tx in a block on rollback', async function () {
    const txHash = randHash();
  ...
  it('keeps a live block\'s mapping when a different, later block whose tx shares the same 8-byte prefix is rolled back', async function () {
    const txA = randHash();
    ...
    // Roll back only blockB (the later block), as a real reorg walk would:
    // it never rolls back blockA without first rolling back blockB.
    await db.beginTransaction();
    ...
    // txA's mapping survives: rolling back blockB must not delete the T/X
    // records the shared 8-byte prefix's later insert overwrote them with,
    // let alone txA's own untouched exact-index record.
    const hitAAfter = await db.getTxBlock(txA);
    expect(hitAAfter).to.not.be.null;
    expect(hitAAfter.block_hash).to.equal(blockA);
  });

  it('returns null for a txid that was never indexed', async function () {
    expect(await db.getTxBlock(randHash())).to.be.null;
  });

  it('returns null for a malformed txid instead of throwing', async function () {
  ...
  it('returns null for a legacy pre-migration record with no stored full txid', async function () {
    const txHash = randHash();
    const blockHash = randHash();
  ...
  it('reports the store\'s own committed tip even when it is stale relative to the found tx', async function () {
    const txHash = randHash();
    const blockHash = randHash();
  ...
  it('returns null when the tx\'s own block record no longer exists', async function () {
    const txHash = randHash();
    const blockHash = randHash();
