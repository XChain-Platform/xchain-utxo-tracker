...
async function setupMempoolStress() {
  resetTxCounter();
...
async function reportMempoolStress() {
  metrics.printTable();
  metrics.saveIfRequested();
  await closeTracker(tracker);
}

async function measureMempoolPopulation() {
...
async function measurePopulatedMempoolQueries() {
...
async function measureMempoolChurn() {
  const roundTimes = [];
...
async function measureMempoolMemory() {
  await clearMempoolDb(tracker);
...
describe('Perf: Mempool Stress', function () {
  before(setupMempoolStress);
  after(reportMempoolStress);

  it('measures mempool population time', measureMempoolPopulation);
  it('measures balance query time with populated mempool', measurePopulatedMempoolQueries);
  it('measures mempool churn performance over 5 rounds', measureMempoolChurn);
  it('measures mempool memory consumption', measureMempoolMemory);
});
