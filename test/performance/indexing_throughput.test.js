...
async function setupSustainedIndexing() {
  resetTxCounter();
...
async function reportAndCloseTracker() {
  metrics.printTable();
  metrics.saveIfRequested();
  await closeTracker(tracker);
}

async function processBatchesAndReportThroughput() {
...
async function detectThroughputDegradation() {
  await closeTracker(tracker);
  tracker = await createTestTracker();
  resetTxCounter();
...
async function checkSustainedIndexingMemory() {
  await closeTracker(tracker);
  tracker = await createTestTracker();
  resetTxCounter();
...
describe('Perf: Sustained Indexing Throughput', function () {
  before(setupSustainedIndexing);
  after(reportAndCloseTracker);

  it('processes blocks in 100-block batches and reports throughput', processBatchesAndReportThroughput);
  it('detects throughput degradation between first and second half', detectThroughputDegradation);
  it('does not leak memory during sustained indexing', checkSustainedIndexingMemory);
});

async function setupSpikeLoad() {
  resetTxCounter();
  tracker = await createTestTracker();
  addressPool = generateAddressPool(SCALE.addresses);
...
async function handleSpikeAndRecovery() {
...
async function checkSpikeScaling() {
  await closeTracker(tracker);
  tracker = await createTestTracker();
  resetTxCounter();
...
describe('Perf: Spike Load', function () {
  before(setupSpikeLoad);
  after(reportAndCloseTracker);

  it('handles spike blocks and recovers to baseline throughput', handleSpikeAndRecovery);
  it('spike block processing scales linearly with transaction count', checkSpikeScaling);
});
