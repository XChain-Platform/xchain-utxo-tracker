...
### Fixed
- `src/XChainUtxoTracker.js` — the reorg-handling branch in `start()` no longer strands aged-out K/M cleanup entries on disk. When a reorg is detected mid-batch the in-flight transaction is rolled back with `endTransaction(false)`, which also discarded the `P_PENDING_CLEANUP_KEY` record listing blocks that had aged out during the batch and still needed K/M cleanup. Those blocks are too old to reorg and genuinely need cleaning, but the rollback erased the only record of them, so neither the normal cleanup cycle nor the startup crash-recovery path would ever process them — their K/M LevelDB entries accumulated indefinitely, growing the SST files and compaction overhead. The branch now persists the pending list with a standalone `db.put(P_PENDING_CLEANUP_KEY, …)` on the underlying store, deliberately outside the rolled-back transaction, so the existing startup recovery path runs `cleanupAgedBlocks()` for those blocks on the next restart. The entries are functionally inert (they guard blocks too old to reorg) and mainnet reorgs are rare, so this is a disk-space reclamation fix with no correctness impact; `cleanupAgedBlocks()` is idempotent (LevelDB deletes of absent keys are no-ops), so routing the reorg case through the same recovery state a crash would produce is safe.
...
### Added
- `src/XChainUtxoTracker.js` — `getUtxosAddress()` now rejects any output whose resolved transaction id is not a full 64-character hash. The output (`O`) record stores the full 32-byte transaction hash, but a record written before that field existed stores a zero hash, which decodes to a null full txid; the resolver then falls back to the 8-byte LevelDB key prefix — a 16-character string that is not a spendable txid. Previously this truncated value was returned and forwarded to the encoder, where `psbt.addInput({ hash })` would build a silently malformed PSBT (a lost-funds risk on signing). The resolver now throws a descriptive error (naming the offending record and instructing the operator to re-index) for both the confirmed and mempool output paths, so a pre-migration database surfaces as an explicit failure rather than corrupting a downstream transaction. All current insertion paths supply the full hash, so a correctly-synced database is unaffected; the new behavior only fires on databases that predate the full-hash output format. `src/LevelUpDb.js` `encodeOutput`/`decodeOutput` gained inline comments documenting the zero-hash sentinel and the re-index requirement, and `README.md` gained an **Upgrading** section spelling out that pre-migration snapshots must be wiped and re-synced. Covered by a new unit test asserting the guard fires on a record with no full hash.
- `src/api.js` — the JSON-RPC `get_sync_status` response now carries a `synced` boolean alongside the existing `lag` value. It is computed against the tracker's own `SYNCED_THRESHOLD` (now exported from `src/XChainUtxoTracker.js`) as `lag !== null && lag <= SYNCED_THRESHOLD`, so a not-yet-indexed tracker (null lag) reports `synced: false`. This lets callers delegate the lag-vs-threshold judgment to the tracker — the authoritative owner of the threshold — instead of each replicating the constant locally and risking silent divergence if the threshold is ever retuned for chain-specific block times. Existing fields are unchanged.
