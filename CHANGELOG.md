...
### Fixed
- Validated bootstrap archives before replacing local data, batched block reassembly, and cleared stale sync state when the node tip falls behind.


## [0.20.0] - 2026-09-17

### Fixed
- Regtest synchronization no longer treats `verificationprogress` as a reliable sync verdict.
...
### Fixed
- The reorg rollback is bounded to the surviving undo window, and a short window is reported at boot.
...
### Fixed
- The confirmations floor comes from the canonical coin registry.
- Fatal paths write a record a collector can read.
